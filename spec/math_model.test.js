...
it('MathModel#angle can calculate angles correctly', () => {
  var _mm = new MathModel()
...
it('MathModel#isStraight can deduce straight lines', () => {
  var _mm = new MathModel()
...
it('MathModel#isHorizontal can deduce horizontal lines', () => {
  var _mm = new MathModel()
  expect(_mm.isHorizontal([{x: -1, y: 3},{x: 1,y: 3.1},{x: 3,y: 3.6},{x: 5,y: 3.3},
     {x: 7, y:2.9}, {x: 9, y:2.8}],0.7)).toEqual(true)
});

it('MathModel#isHorizontal can deduce non-horizontal lines', () => {
  var _mm = new MathModel()
...
it('MathModel#isVertical can deduce vertical lines', () => {
  var _mm = new MathModel()
...
it('MathModel#isVertical can deduce vertical lines', () => {
  var _mm = new MathModel()
  expect(_mm.isVertical([{x: 50, y: -1},{x: 48,y: 1},{x: 46,y: 3},{x: 51,y: 5},
     {x: 53, y:7}, {x: 50, y:9}],5)).toEqual(true)
});

it('MathModel#isVertical can deduce non-vertical lines', () => {
  var _mm = new MathModel()
